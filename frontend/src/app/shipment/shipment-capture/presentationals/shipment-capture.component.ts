...
import {CompleteTaskEvent} from "./events/complete-task.event";
...
export class ShipmentCaptureComponent implements OnInit, DoCheck {
  ...
  @Output()
  public completeTaskEvent: EventEmitter<CompleteTaskEvent> = new EventEmitter<CompleteTaskEvent>();
  ...
  public initalizeShipment(): ShipmentResource {
  ...
  public createShipment() {
    const shipment = this.initalizeShipment();
  ...
  public saveShipment() {
    ...
    this.completeTaskEvent.emit(
      new CompleteTaskEvent(shipment.trackingId, "Change shipment order")
    );
